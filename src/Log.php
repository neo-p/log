...
class Log extends Style
{
    ...
    public static function info($data = "")
    {
        $type = __FUNCTION__;
        if (is_string($data)) {
            self::stdout('> [Info][' . date('Y-m-d H:i:s') . ']: ' . $data, 0, self::MODE_DEFAULT, '', '', $type);
    ...
    public static function debug($data = "")
    {
        $type = __FUNCTION__;
        if (is_string($data)) {
            self::stdout('> [Debug][' . date('Y-m-d H:i:s') . ']: ' . $data, 0, self::MODE_DEFAULT, self::FG_WHITE, '', $type);
        } else if (\is_array($data) || is_object($data)) {
            self::formatObject($data, function (string $string = '') use ($type) {
    ...
    public static function warning($data = "")
    {
        $type = __FUNCTION__;
        if (is_string($data)) {
            self::stdout('> [Warning][' . date('Y-m-d H:i:s') . ']: ' . $data, 0, self::MODE_DEFAULT, self::FG_CYAN, '', $type);
        } else if (\is_array($data) || is_object($data)) {
            self::formatObject($data, function (string $string = '') use ($type) {
    ...
    public static function error($data = "")
    {
        $type = __FUNCTION__;
        if (is_string($data)) {
            self::stdout('> [Error][' . date('Y-m-d H:i:s') . ']: ' . $data, 0, self::MODE_DEFAULT, self::FG_RED, '', $type);
        } else if (\is_array($data) || is_object($data)) {
            self::formatObject($data, function (string $string = '') use ($type) {
